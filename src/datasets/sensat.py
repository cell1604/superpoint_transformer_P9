...
from src.datasets.sensat_config import *
...
def read_sensat_tile(
       filepath, xyz=True, rgb=True, semantic=False, remap=False):
   data = Data()
#    key = 'testing'
   with open(filepath, "rb") as f:
       ...
       if xyz:
            ...
            data.pos = torch.stack([
                torch.FloatTensor(tile["vertex"][axis])
                for axis in ["x", "y", "z"]], dim=-1)
            print("XYZ data shape:", data.pos.shape)
            print("XYZ data matrix:")
            print(data.pos)
       ...
       if rgb:
            ...
            data.rgb = to_float_rgb(torch.stack([
                torch.ByteTensor(tile["vertex"][axis])
                for axis in ["red", "green", "blue"]], dim=-1)) 
            print("RGB data shape:", data.rgb.shape)
            print("RGB data matrix:")
            print(data.rgb)
...
#                tile[key]['intensity']).clip(min=0, max=60000) / 60000
       ...
       if semantic:
            ...
            y = torch.ByteTensor(tile["vertex"]['class'])
            data.y = torch.from_numpy(ID2TRAINID)[y] if remap else y
            print("Semantic data shape:", data.y.shape)
            print("Semantic data matrix:")
            print(data.y)
...
# def read_sensat_tile(
#        filepath, xyz=True, rgb=False, semantic=False, remap=False):
#    data = Data()
# #    key = 'testing'
#    with open(filepath, "rb") as f:
#        tile = PlyData.read(f)
...
class SENSAT(BaseDataset):
    ...
    def read_single_raw_cloud(self, raw_cloud_path):
        ...
        """
        return read_sensat_tile(
            raw_cloud_path, xyz=True, rgb=True,  semantic=True)
...
class MiniSENSAT(SENSAT):
    """A mini version of SENSAT with only a few windows for
    experimentation.
    """
    _NUM_MINI = 2
